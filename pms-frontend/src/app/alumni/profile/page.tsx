...
const Profile = () => {
  ...
  useEffect(() => {
    const fetchProfile = async () => {
      try {
        ...
        const response = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/alumni/get-user/${user._id}`)
  ...
  const handleSubmit = async () => {
    try {
      ...
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/alumni/update/${profile._id}`, {
        method: 'PATCH',
        headers: {
  ...
  return (
    <div className="container mx-auto py-6">
      <Card>
        <CardHeader>
          <CardHeader>Alumni Profile</CardHeader>
        </CardHeader>
        <CardBody>
